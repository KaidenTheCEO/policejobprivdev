...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob and not exports["qb-policejob"]:IsCopCL() then
        if DutyBlips then
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    ...
    if (PlayerJob ~= nil) and not exports["qb-policejob"]:IsCopCL() then
        if DutyBlips then
            for k, v in pairs(DutyBlips) do
                RemoveBlip(v)
            end
...
RegisterNetEvent('police:client:UpdateBlips', function(players)
    if PlayerJob and (exports["qb-policejob"]:IsCopCL() or PlayerJob.name == 'ambulance') and
        onDuty then
        if DutyBlips then
            for k, v in pairs(DutyBlips) do
                RemoveBlip(v)
