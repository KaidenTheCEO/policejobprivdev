...
local function UpdateBlips()
    ...
    for k, v in pairs(players) do
        for i, j in pairs(Config.PoliceJobs) do
            if (v.PlayerData.job.name == j or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
...
local function GetCurrentCops()
    ...
    for k, v in pairs(players) do
        for i, j in pairs(Config.PoliceJobs) do
            if v.PlayerData.job.name == j and v.PlayerData.job.onduty then
                amount = amount + 1
            end
        end
    end
...
QBCore.Commands.Add("spikestrip", "Place Spike Strip (Police Only)", {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
        if exports["qb-policejob"]:IsCopSV(src) then
...
QBCore.Commands.Add("pobject", "Place/Delete An Object (Police Only)", {{name = "type",help = "Type object you want or 'delete' to delete"}}, true, function(source, args)
    ...
    if exports["qb-policejob"]:IsCopSV(src) and Player.PlayerData.job.onduty then
        if type == "pion" then
...
QBCore.Commands.Add("paylawyer", "Pay Lawyer (Police, Judge Only)", {{name = "id",help = "ID of the player"}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if exports["qb-policejob"]:IsCopSV(src) or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
        if OtherPlayer then
...
QBCore.Commands.Add("takedna", "Take a DNA sample from a person (empty evidence bag needed) (Police Only)", {{name="id", help="ID of the person"}}, true, function(source, args)
    ...
    if ((exports["qb-policejob"]:IsCopSV(src)) and Player.PlayerData.job.onduty) and OtherPlayer then
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    ...
    if Player.Functions.GetItemByName(item.name) then
        if item.info and item.info ~= "" then
            if not exports["qb-policejob"]:IsCopSV(src) then
...
QBCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(source, cb)
    ...
    for k, v in pairs(players) do
        for i, j in pairs(Config.PoliceJobs) do
            if v.PlayerData.job.name == j and v.PlayerData.job.grade.level >= 2 then
                retval = true
                break
            end
        end
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if CuffedPlayer then
        if Player.Functions.GetItemByName("handcuffs") or exports["qb-policejob"]:IsCopSV(src) then
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    if EscortPlayer then
        if (exports["qb-policejob"]:IsCopSV(src) or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
...
RegisterNetEvent('police:server:BillPlayer', function(playerId, price)
    ...
    if exports["qb-policejob"]:IsCopSV(src) then
        if OtherPlayer then
            ...
            TriggerEvent('qb-bossmenu:server:addAccountMoney', Player.PlayerData.job.name, price)
